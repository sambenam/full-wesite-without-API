...
function applyStaffProfileChanges(staffId, changes) {
  ...
  pushAdminNotification("staff", "تغییر پروفایل عضو مدیریت", `«${staff.name}» ${fields.map(field => profileLabels[field]).join("، ")} را به‌روزرسانی کرد.`, { "کاربر": staff.name, "شناسه کاربر": "#" + toPersianDigits(staff.id), "نقش": staff.role === "ادمین" ? "ادمین" : "مدیر", "موارد تغییرکرده": fields.map(field => profileLabels[field]).join("، "), "جزئیات تغییرات": fields.map(field => `${profileLabels[field]}: ${staff[field] || "—"} → ${changed[field]}`).join("| "), "زمان تغییر": new Date().toLocaleString("fa-IR"), "نشانی IP": "در انتظار اتصال به سرور" });
}
...
function openNotificationDetails(id) {
  ...
  else if (n.type === "user") body.innerHTML = `<section class="signup-account-card"><div class="signup-account-head"><span class="signup-account-avatar"><i class="fas fa-user"></i></span><div><small>حساب کاربری تازه ثبت‌شده</small><strong>${details["نام کاربر"] || "کاربر جدید"}</strong><em>${details["شناسه کاربر"] || "—"}</em></div><span class="signup-account-status">${details["وضعیت حساب"] || "فعال"}</span></div><div class="signup-contact"><div><i class="fas fa-envelope"></i><small>ایمیل</small><b>${details["ایمیل"] || "—"}</b></div><div><i class="fas fa-phone"></i><small>تلفن</small><b>${details["تلفن"] || "—"}</b></div></div><div class="signup-account-meta"><span>نقش: <b>${details["نقش"] || "کاربر عادی"}</b></span><span>روش ثبت‌نام: <b>${details["روش ثبت‌نام"] || "—"}</b></span><span>زمان عضویت: <b>${details["تاریخ ثبت‌نام"] || "—"}</b></span></div></section>`;
  else if (n.type === "staff") { const auditRows = (details["جزئیات تغییرات"] || "").split("| ").filter(Boolean).map(row => { const parts=row.split(" → "); return `<div class="profile-audit-row"><span><small>قبل از تغییر</small><b>${parts[0] || "—"}</b></span><i class="fas fa-arrow-left"></i><span class="after"><small>پس از تغییر</small><b>${parts[1] || "—"}</b></span></div>`; }).join(""); body.innerHTML = `<section class="profile-audit-card"><div class="profile-audit-head"><i class="fas fa-user-gear"></i><div><small>ثبت تغییر در پروفایل ${details["نقش"] || "عضو مدیریت"}</small><strong>${details["کاربر"] || "—"}</strong><em>شناسه: ${details["شناسه کاربر"] || "—"}</em></div></div><div class="profile-audit-fields"><small>فیلدهای تغییرکرده</small><strong>${details["موارد تغییرکرده"] || "—"}</strong></div>${auditRows || '<p class="notification-detail-empty">جزئیات مقدارهای تغییرکرده از سرور دریافت نشده است.</p>'}<div class="profile-audit-footer"><span><i class="fas fa-clock"></i> ${details["زمان تغییر"] || "—"}</span><span><i class="fas fa-network-wired"></i> ${details["نشانی IP"] || "—"}</span></div></section>`; }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  setTimeout(function () {
    ...
    pushAdminNotification("staff", "ویرایش پروفایل ادمین", "ادمین «محمد رضایی» اطلاعات پروفایل خود را به‌روزرسانی کرد.", { "کاربر": "محمد رضایی", "شناسه کاربر": "#۲", "نقش": "ادمین", "موارد تغییرکرده": "نام کاربری و تلفن همراه", "جزئیات تغییرات": "نام کاربری: محمد رضایی → محمد رضایی‌منش| تلفن همراه: ۰۹۱۲۲۲۲۲۲۲۲ → ۰۹۱۲۳۴۵۶۷۸۹", "زمان تغییر": "۱۴۰۵/۰۵/۰۳ - ۱۰:۴۵", "نشانی IP": "185.73.12.44" });
...
function renderStaffTable() {
  ...
  const addStaffBtn = document.querySelector("#view-staff .btn-primary"); if (addStaffBtn) { const locked = currentNotificationRole() === "ادمین"; addStaffBtn.style.display = locked ? "none" : "inline-flex"; }
...
function editStaff(id) {
  if (currentNotificationRole() === "ادمین") { showToast("ادمین‌ها فقط اجازه مشاهده فهرست مدیران و ادمین‌ها را دارند.", "error"); return; }
